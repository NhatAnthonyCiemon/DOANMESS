...
import com.example.doanmess.adapters.Chat_AllChatAdapter
import com.example.doanmess.models.DataMess
import com.example.doanmess.models.DataMessGroup
import com.example.doanmess.activities.MainChat
import com.example.doanmess.models.MessageGroupSQL
import com.example.doanmess.MessageGroupSQLDao
                                            ...
                                            if (document != null) {
                                                ...
                                                if (existingItem != null) {
                                                    existingItem.apply {
                                                        ...
                                                        this.avatar = avatar
                                                        this.name = name
                                                        this.message = if (!othersend) "${atvtContext.getString(
                                                            R.string.You
                                                        )}: ${content.toString()}" else last_name + ": ${content.toString()}"
                                ...
                                if (User!!.uid == sendId) {
                                    ...
                                    if (existingItem != null) {
                                        ...
                                        if (listOff.contains(childSnapshot.key.toString())) {
                                            list.add(
                                                DataMessGroup(avatarList[childSnapshot.key.toString()].toString(), childSnapshot.key!!, myGroup[childSnapshot.key.toString()].toString(), content.toString(), timestamp!!, status, "${atvtContext.getString(
                                                    R.string.You
                                                )}", myGroup[childSnapshot.key.toString()].toString(), false)
                                            )
                                        } else {
                                            list.add(
                                                DataMessGroup(avatarList[childSnapshot.key.toString()].toString(), childSnapshot.key!!, myGroup[childSnapshot.key.toString()].toString(), content.toString(), timestamp!!, status, "${atvtContext.getString(
